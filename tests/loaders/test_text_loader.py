...
def test_text_loader_csv_norm_and_hop():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        pd.DataFrame(arr).to_csv(
            os.path.join(sess, "text_embeddings.csv"), header=False, index=False
        )

        loader = TextLoader(
            embedding_glob="text_embeddings.csv",
            normalize=True,
            frame_hop=3,
            cache=False,
        )
...
def test_missing_files_raise():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        loader = TextLoader(
            embedding_glob="does_not_exist.npy", transcript_glob="nope.txt"
        )
        import pytest
